...
void initGraph()
{
	...
	St = 0, En = (N+1) * 2 + 1;
	addEdge(St, 1, K, 0);
	for(int i=1; i<=N; i++) addEdge(St, i+1, 1, 0), addEdge(i+1+N+1, En, 1, 0);
	for(int i=1; i<=N; i++) for(int j=i+1, x; j<=N+1; j++)
		scanf("%d", &x), addEdge(i, j+N+1, 1, x);
}
void deleteGraph()
{
	for(int i=St; i<=En; i++)
